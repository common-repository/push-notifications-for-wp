...
class App_Subscribers_Table extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
			'display_name' => __('Username', 'push-notifications-for-wp'),
			'user_email' => __('E-mail', 'push-notifications-for-wp'),
	...
	function prepare_items() {
		...
		global $wpdb;
		$sql = "SELECT * FROM $wpdb->users as users LEFT JOIN $wpdb->usermeta as usermeta on ID = user_id WHERE 1=1";
		$sql_count = "SELECT COUNT(ID) FROM $wpdb->users as users LEFT JOIN $wpdb->usermeta as usermeta on ID = user_id WHERE 1=1";

		$filter = $wpdb->prepare(" AND meta_key = 'wp_capabilities' AND meta_value LIKE %s", '%' . PNFW_Push_Notifications_for_WordPress_Lite::USER_ROLE . '%');
		$sql .= $filter;
		$sql_count .= $filter;
		...
		$sql .= $wpdb->prepare(" ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged * $per_page);
	...
	function column_default($item, $column_name) {
		switch ($column_name) {
			...
			case 'devices':
				...
				$devices = $wpdb->get_col($wpdb->prepare("SELECT os FROM $push_tokens WHERE user_id=%d", $item->ID));
