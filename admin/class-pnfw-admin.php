...
final class PNFW_Admin {
	...
	function render_meta_box($post) {
		...
		if (get_post_meta($post->ID, 'pnfw_do_send_push_notifications_for_this_post', true)) {
			$send_push_notifications_checkbox = true;
		} else {
			$send_push_notifications_checkbox = (!in_array($post->post_status, array('publish', 'private', 'trash', 'inherit'), true)); // Post is scheduled, incomplete, being edited, or is awaiting publication
		}

		?>

		<label>
			<input type="checkbox" <?php echo ($send_push_notifications_checkbox ? ' checked="checked"' : null) ?> value="1" name="pnfw_do_send_push_notifications_for_this_post" id="pnfw-do-send-push-notifications-for-this-post" />
			<?php $post_type_object = strtolower(get_post_type_object($post->post_type)->labels->singular_name); ?>
			<?php if ($post->post_status === 'publish') {
				echo sprintf(__('Send for this %s on update', 'push-notifications-for-wp'), $post_type_object);
			} else {
				echo sprintf(__('Send for this %s on publish', 'push-notifications-for-wp'), $post_type_object);
			} ?>
		</label>
	...
	function save_postdata($postid) {
		...
		if (isset($_POST['pnfw_do_send_push_notifications_for_this_post'])) {
			add_post_meta($postid, 'pnfw_do_send_push_notifications_for_this_post', true, true);
		}
		else {
			delete_post_meta($postid, 'pnfw_do_send_push_notifications_for_this_post');
		}
