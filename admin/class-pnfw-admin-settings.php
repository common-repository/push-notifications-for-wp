...
class PNFW_Admin_Settings {
	public static function output() { ?>
		...
		$pnfw_ios_key_id = get_option('pnfw_ios_key_id');
		$pnfw_ios_team_id = get_option('pnfw_ios_team_id');
		$pnfw_ios_private_key_content = get_option('pnfw_ios_private_key_content');
		$pnfw_ios_bundle_id = get_option('pnfw_ios_bundle_id');
	...
	private static function save_ios_options() {
		delete_transient('pnfw_ios_auth_token');

		$pnfw_ios_key_id = pnfw_get_post('pnfw_ios_key_id');
		update_option('pnfw_ios_key_id', $pnfw_ios_key_id);

		$pnfw_ios_team_id = pnfw_get_post('pnfw_ios_team_id');
		update_option('pnfw_ios_team_id', $pnfw_ios_team_id);

		$pnfw_ios_private_key_content = isset($_POST['pnfw_ios_private_key_content']) ? sanitize_textarea_field($_POST['pnfw_ios_private_key_content']) : false;
		update_option('pnfw_ios_private_key_content', $pnfw_ios_private_key_content);

		$pnfw_ios_bundle_id = pnfw_get_post('pnfw_ios_bundle_id');
		update_option('pnfw_ios_bundle_id', $pnfw_ios_bundle_id);
	...
	private static function print_ios_box($pnfw_ios_push_notifications,
		...
		<div class="postbox">
			<div class="inside">
				...
				<table class="form-table">
					<tbody id="pnfw_section_auth_token">
					<tr valign="top">
						<th scope="row">
							<label for="pnfw_ios_bundle_id"><?php _e('Bundle ID', 'push-notifications-for-wp'); ?></label>
						</th>
						<td>
							<input type="text" class="regular-text" name="pnfw_ios_bundle_id" id="pnfw_ios_bundle_id" value="<?php echo $pnfw_ios_bundle_id; ?>" maxlength="255" />
						</td>
					</tr>

					<tr valign="top">
						<th scope="row">
							<label for="pnfw_ios_key_id"><?php _e('Key ID', 'push-notifications-for-wp'); ?></label>
						</th>
						<td>
							<input type="text" class="regular-text" name="pnfw_ios_key_id" id="pnfw_ios_key_id" value="<?php echo $pnfw_ios_key_id; ?>" maxlength="10" />
						</td>
					</tr>

					<tr valign="top">
						<th scope="row">
							<label for="pnfw_ios_team_id"><?php _e('Team ID', 'push-notifications-for-wp'); ?></label>
						</th>
						<td>
							<input type="text" class="regular-text" name="pnfw_ios_team_id" id="pnfw_ios_team_id" value="<?php echo $pnfw_ios_team_id; ?>" maxlength="10" />
						</td>
					</tr>

					<tr valign="top">
						<th scope="row">
							<label for="pnfw_ios_private_key_content"><?php _e('Key Content', 'push-notifications-for-wp'); ?></label>
						</th>
						<td>
							<textarea class="large-text code" name="pnfw_ios_private_key_content" id="pnfw_ios_private_key_content" rows="7" cols="80"><?php echo $pnfw_ios_private_key_content; ?></textarea>
							<p><a href="https://products.delitestudio.com/wordpress/push-notifications-for-wordpress/configuring-ios-push-notifications/" target="_blank"><?php _e('Obtaining the private key', 'push-notifications-for-wp'); ?></a></p>
						</td>
					</tr>
					</tbody>
	...
	private static function print_misc_box($pnfw_use_wpautop,
		$pnfw_disable_email_verification,
		$pnfw_uninstall_data,
		$pnfw_force_badge_count) { ?>
		<!-- postbox -->
	...
	private static function print_ios_notices($pnfw_enable_push_notifications,
		...
		if ($pnfw_enable_push_notifications && $pnfw_ios_push_notifications) {
			if (curl_version()['version_number'] < 0x72e00) { ?>
				<div id="message" class="error"><p><?php _e('Required libcurl >= 7.46.0', 'push-notifications-for-wp'); ?></p></div>
			<?php }

			if ((curl_version()['features'] & CURL_VERSION_HTTP2) === 0) { ?>
				<div id="message" class="error"><p><?php _e('Missing HTTP/2 support for libcurl', 'push-notifications-for-wp'); ?></p></div>
			<?php }

			if (!$pnfw_ios_key_id) { ?>
				<div id="message" class="error"><p><?php _e('Missing Key ID', 'push-notifications-for-wp'); ?></p></div>
			<?php }

			if (!$pnfw_ios_team_id) { ?>
				<div id="message" class="error"><p><?php _e('Missing Team ID', 'push-notifications-for-wp'); ?></p></div>
			<?php }

			if (!$pnfw_ios_private_key_content) { ?>
				<div id="message" class="error"><p><?php _e('Missing Private Key', 'push-notifications-for-wp'); ?></p></div>
			<?php }

			if (!$pnfw_ios_bundle_id) { ?>
				<div id="message" class="error"><p><?php _e('Missing Bundle ID', 'push-notifications-for-wp'); ?></p></div>
			<?php }
		}
	}
	...
	private static function expiration_date($media_id, $passphrase = NULL) {
